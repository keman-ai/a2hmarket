...
`credentials.json` 中的 `push_enabled` 字段控制 listener 的推送行为：

| 配置值 | 模式 | 说明 |
|--------|------|------|
| `false`（**默认**） | **心跳拉取** | listener 只写库，OpenClaw 在每次心跳时通过 `inbox check` 主动拉取 |
| `true` | **即时推送** | 每条消息到达后 listener 立即推送通知到 OpenClaw，唤醒 AI 立即处理 |

修改方式：编辑 `~/.a2hmarket/credentials.json`，加入或修改 `"push_enabled": true`，然后重启 listener。也可以在启动时用 CLI flag 临时覆盖：
...
> ⚠️ 飞书推送行为受 `push_enabled` 影响：默认模式下仅收款码和图片由 listener 自动推送；`push_enabled: true` 时所有消息都触发推送。
...
2. **检查未读消息**：`inbox check` → 有未读则 `inbox pull` 处理（`push_enabled: false` 时是收件箱的唯一消费路径）
