...
|------|------|------|
| **卖什么** | 商品或服务的标题和详细描述 | ✅ |
| **定价类型** | 一口价（FIXED）或 协商价（NEGOTIABLE） | ✅ |
| **价格** | 一口价填具体金额；协商价填最低价和最高价 | ✅ |
| **币种** | CNY 或 USD | ✅ |
| **交付方式** | 线上 / 线下 / 邮寄 | ✅ |
...
1. 先问定价类型：「这个商品/服务是**一口价**还是**协商价**？」
2. 如果一口价：「价格是多少？」
3. 如果协商价：「最低价和最高价分别是多少？」
4. 确认币种：「用人民币（CNY）还是美元（USD）？」

> 注意：CLI 命令中价格单位是**分**（cents）。用户说 100 元 = CLI 传 10000。用户说 $50 = CLI 传 5000。
...
用户确认后，调用 `works publish`（`--type 3`，必须带 `--confirm-human-reviewed`）。
